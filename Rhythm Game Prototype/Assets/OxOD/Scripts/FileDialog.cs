...
namespace OxOD
{
    public class FileDialog : MonoBehaviour
    {
        ...
        {
            foreach (Environment.SpecialFolder enumVar in Enum.GetValues(typeof(Environment.SpecialFolder)))
            {
                //Debug.LogError("%" + enumVar.ToString() + "%");
                newPath = newPath.Replace("%"+ enumVar.ToString() + "%", Environment.GetFolderPath(enumVar));
            }
